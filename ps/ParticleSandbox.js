...
class ParticleSandbox {
    ...
    updatePairLocation(pair, root) {
        if (root !== this.pairs[2] && pair.distance > (pair.particle1.radius * pair.particle2.radius * 200)) {
            if (root) root.remove(pair)
            this.pairs[2].add(pair)
        } else if (root !== this.pairs[1] && pair.distance > (pair.particle1.radius * pair.particle2.radius * 50)) {
            if (root) root.remove(pair)
            this.pairs[1].add(pair)
        } else if (root !== this.pairs[0]) {
            if (root) root.remove(pair)
